...
func Test_cache_client(t *testing.T) {
	...
	t.Run("update objects", func(t *testing.T) {
		...
		t.Run("update - no changes", func(t *testing.T) {
			...
			err := c.Get("foo", types.NamespacedName{Name: "bar"}, objBefore)
			require.NoError(t, err)

			err = c.Update("foo", objBefore)
			require.NoError(t, err)

			objAfterUpdate := &cloudv1.CloudMachine{}
			err = c.Get("foo", types.NamespacedName{Name: "bar"}, objAfterUpdate)
			require.NoError(t, err)

			require.Equal(t, objAfterUpdate, objBefore, "obj before and after must be the same")
		...
		t.Run("update - with changes", func(t *testing.T) {
			...
			require.NoError(t, err)
			objBefore.Labels = map[string]string{"foo": "bar"}
		...
		t.Run("update - with conflict", func(t *testing.T) {
			createMachine(t, c, "foo", "bazz")

			objBefore := &cloudv1.CloudMachine{}
			err := c.Get("foo", types.NamespacedName{Name: "bazz"}, objBefore)
			require.NoError(t, err)
			objBefore.Labels = map[string]string{"foo": "bar"}
		...
		t.Run("Update with owner references", func(t *testing.T) {
			...
			t.Run("fails if referenced object does not exists", func(t *testing.T) {
				createMachine(t, c, "foo", "child1")
				obj := &cloudv1.CloudMachine{}
				err = c.Get("foo", types.NamespacedName{Name: "child1"}, obj)
				require.NoError(t, err)
			...
			t.Run("updates takes care of ownedObjects", func(t *testing.T) {
				...
				err = c.Get("foo", client.ObjectKeyFromObject(obj), obj)
				require.NoError(t, err)

				obj.OwnerReferences[0].Name = "parent2"
	...
	t.Run("delete objects", func(t *testing.T) {
		...
		t.Run("delete with finalizers", func(t *testing.T) {
			...
			err = c.Get("foo", types.NamespacedName{Name: "baz"}, objBefore)
			require.NoError(t, err)
			require.Zero(t, objBefore.GetDeletionTimestamp())
...
func createMachine(t *testing.T, c *cache, resourceGroup, name string) {
	obj := &cloudv1.CloudMachine{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
