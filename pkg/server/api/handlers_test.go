...
func TestPortForward(t *testing.T) {
	...
	rawConn, err := dialer.DialContextWithAddr(ctx, "kube-apiserver-foo")
	require.NoError(t, err)
	defer rawConn.Close()
...
func createServerAndGetClient(t *testing.T, ctx context.Context) (client.Client, *rest.Config, error) {
	...
	server := mux.NewServer("127.0.0.1", tlsConfig, NewAPIServerHandler(scheme))
	...
	k := clientcmdapi.Config{
		Clusters: map[string]*clientcmdapi.Cluster{
			"goofy": {
				Server:                   fmt.Sprintf("https://localhost:%d/apiserver/test1", service1.Port()),
